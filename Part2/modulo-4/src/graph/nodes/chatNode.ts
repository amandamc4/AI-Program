...
export function createChatNode(llmClient: OpenRouterService, preferencesService: PreferencesService) {
  return async (state: GraphState, runtime?: Runtime): Promise<Partial<GraphState>> => {
    ...
    // Maps what is User messages and what is AI messages to filter later 
    ...
    if (!result.success || !result.data) {
      console.error('❌ Fail to generate response:', result.error);
      return {
        messages: [
          new AIMessage('Sorry, I encountered an error. Can you try again?')
        ]
      }
    }
