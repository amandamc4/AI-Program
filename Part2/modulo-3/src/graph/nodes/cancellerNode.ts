...
export function createCancellerNode(appointmentService: AppointmentService) {
  return async (state: GraphState): Promise<GraphState> => {
    ...
    try {
      //if the state doesn't have the required fields for cancellation, return an error message in the state and set actionSuccess to false and stop the process.
