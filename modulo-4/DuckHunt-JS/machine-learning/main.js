...
export default async function main(game) {
    ...
    setInterval(async () => {
        ...
        // takes a screenshot of the game screen and creates an ImageBitmap from it
        const bitmap = await createImageBitmap(canvas);

        // whenever we need to predict, we need to transform the image into a tensor and send it to the worker for inference
        worker.postMessage({
            type: 'predict',
            image: bitmap,
        }, [bitmap]);
